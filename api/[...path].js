...
// Update step
app.put('/api/categories/:categoryId/phases/:phaseId/steps/:stepId', async (req, res) => {
  try {
    const db = await getDatabase();
...
// Time tracking - log
app.post('/api/time-tracking/log', async (req, res) => {
  try {
    const db = await getDatabase();
    const log = {
      ...req.body,
...
// Time tracking - logs
app.get('/api/time-tracking/logs', async (req, res) => {
  try {
    const db = await getDatabase();
    const { startDate, endDate, userId } = req.query;
